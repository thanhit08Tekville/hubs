...
import { InteractiveSpotLightTag, LightTag } from "../bit-components";
import { SpotLight } from "three";
import { HubsWorld } from "../app";

export type InteractiveSpotLightParams = {
  color?: string;
  intensity?: number;
  range?: number;
  decay?: number;
...
const DEFAULTS: Required<InteractiveSpotLightParams> = {
  color: "#ffffff",
  intensity: 1.0,
  range: 0,
  decay: 2.0,
...
export function inflateInteractiveSpotLight(world: HubsWorld, eid: number, params: InteractiveSpotLightParams) {
  const requiredParams = Object.assign({}, DEFAULTS, params) as Required<InteractiveSpotLightParams>;
  ...
  addComponent(world, InteractiveSpotLightTag, eid);
  return eid;
}
